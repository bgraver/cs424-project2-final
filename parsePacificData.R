...
headerLines = read_lines("hurdat2-nepac-1949-2018-122019.txt")
...
while (i <= length(headerLines)) {
  ...
  if (grepl('^EP', curline)){
    match_string = str_match(curline, '^EP(\\d{2})\\d{4},\\s+(\\S+),\\s+(\\d+)')
    ...
    i = j+1
    if (i > count) {
      print(str_interp('Percentage parsed: ${as.integer(i/length(headerLines)*100)}%'))
      count = count + length(headerLines)/10
    }
  }
}
...
write.csv(parse_df,file='Pacific_hurricane_data.csv',row.names = FALSE)
